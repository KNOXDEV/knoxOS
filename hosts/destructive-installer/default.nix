{ config, pkgs, lib, modulesPath, configToInstall, ... }:
let
  ...
  evaluatedSystem = import (pkgs.path + "/nixos/lib/eval-config.nix") {
    system = "x86_64-linux";
    modules = [ (configToInstall + "/configuration.nix") ];
  };
in
{
  imports = [
  ...
  isoImage.isoName = lib.mkForce "knoxos-auto-installer-${config.system.nixos.label}-${pkgs.stdenv.hostPlatform.system}.iso";
  ...
  # this will let the installer target the disk its executing from,
  # at the cost of needing enough ram to fit the entire installer
  ...
  systemd.services.install = {
    ...
    script = with pkgs; ''
      ...
      cp -r ${configToInstall}/* /mnt/etc/nixos/
